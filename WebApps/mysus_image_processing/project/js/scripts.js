...
const pages = ["home", "gallery", "upload", "process", "about", "logout"]
...
function goTo(page) {
	...
	if (page == "upload") {
		...
		pages.forEach((otherPage) => {
			...
			if (otherPage != "upload" && otherPage != "logout") {document.getElementById(otherPage).classList.add("hidden")}
		})
		if (page != "process") {
...
function updateProgress(evt){
    if (evt.loaded == evt.total) {
        ...
        setTimeout(function () { upload_btn.classList.replace("btn-success", "btn-accent"); upload_btn.classList.add("btn-outline"); window.location.reload(); }, 1000);
	}
}
