...
function getCollectionsData() {
    ...
    $.get("/cromos/getImages", {"token": getCookie("token")})
    .done(function( data ) {
    ...
    /*$.get("/cromos/getCollections", {"token": getCookie("token")})
    .done(function( data ) {
        const info = JSON.parse(data)
        let collectionsContainer = document.getElementById("COLLECTIONS_CONTAINER");

        if (info.data.length === 0) {
            collectionsContainer.innerHTML = "<h6>Nao existem colecoes disponiveis.</h6>";
        } else {
            info.data.forEach(element => {
                collectionsContainer.innerHTML += collectionTemplate.replace("COLLECTION_NAME", element[1])
                                                                    .replace("COLLECTION_ID", element[0])
                                                                    .replace("IMG_SOURCE", possibleImgs[Math.floor(Math.random() * possibleImgs.length)]);
            });
        }
    })
    .fail(function() {
